...
```
nohdol-harness/
├── AGENTS.md              # 단일 원본 — 레지스트리·가드레일·라우팅·진화 트리거·티어 매핑
...
1. **라우팅**: 요청을 받으면 `AGENTS.md`의 프로젝트 레지스트리에서 관련 프로젝트를 식별하고 해당 하네스를 로드한다.
2. **단일 프로젝트** 작업은 그 프로젝트 하네스로 직접, **다중 프로젝트** 작업은 `orchestrate` 스킬로 팀을 구성해 진행한다 (위임 깊이 최대 2단계).
