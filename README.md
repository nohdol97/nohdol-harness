...
│   ├── hooks/             # 실행 계층 — tdd-gate.py (커밋 시점 TDD 강제, ADR 008·014·015), agentsview-daemon.py (동기화 데몬 자동 기동), harness-review-reminder.py (일일 경량·주간 전체 점검 자동 트리거)
│   ├── githooks/          # 전역 core.hooksPath 대상 — tdd-gate 진입 shim + 로컬 훅 체인 (도구 무관 강제, 등록은 harness-install 1단계)
