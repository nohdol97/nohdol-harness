...
- **User comprehension is completion (§13)**: correct output alone is not delivery. For PR-bound behavior work, dispatch background review and run the comprehension quiz in that review window; wrong or skipped answers hold the PR. Rework that changes behavior gets a delta quiz. A **normative** root change committed straight to main is quizzed before `git push`, and a wrong or skipped answer holds the push — the one place §13 reaches non-code work. Provide a diff reading guide and teach non-obvious decisions.
