...
- **Orchestration invariants (§7, ADR 028·032)**: implementation continuations, work-tracker/carryover resumes, and diagnosis→first product Edit all re-enter `orchestrate`. K8s/IaC goes through `infra-specialist`. Product changes of 3+ files use `implementer`; ≤2 low-risk files may be direct but still require independent review — **off on a `사내` profile (§13-3), where the main loop verifies and records the exemption instead**. Collection uses `explorer`, causation uses `troubleshooter`, verification uses `reviewer`. Record the expected Agent-call budget, reuse the same agent for delta work, and add agents only for a new independent risk axis or required independence. Dispatch independent work simultaneously; background is the default, with foreground only for ≲1-minute lookups needed in the same response.
...
- **User comprehension is completion (§13)**: correct output alone is not delivery. For PR-bound behavior work, dispatch background review and run the comprehension quiz in that review window — **on a `사내` profile that dispatch is off (§13-3, ADR 038) and the quiz runs at PR finish instead**; wrong or skipped answers hold the PR either way. Rework that changes behavior gets a delta quiz. The quiz is a sub-project gate (`project/<name>/` **and its worktrees**) — **harness assets are exempt on every path** (root files, `.agents/**`, root `docs/**`, the root remote-PR cycle included). Provide a diff reading guide and teach non-obvious decisions.
