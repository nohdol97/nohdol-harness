...
- **User comprehension is completion (§13)**: correct output alone is not delivery. For PR-bound behavior work, dispatch background review and run the comprehension quiz in that review window; wrong or skipped answers hold the PR. Rework that changes behavior gets a delta quiz. The quiz is a sub-project gate (`project/<name>/` **and its worktrees**) — **harness assets are exempt on every path** (root files, `.agents/**`, root `docs/**`, the root remote-PR cycle included). Provide a diff reading guide and teach non-obvious decisions.
