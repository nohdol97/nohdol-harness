...
- **Orchestration invariants (§7, ADR 028·032)**: implementation continuations, work-tracker/carryover resumes, and diagnosis→first product Edit all re-enter `orchestrate`. K8s/IaC goes through `infra-specialist`. Product changes of 3+ files use `implementer`; ≤2 low-risk files may be direct but still require independent review. Collection uses `explorer`, causation uses `troubleshooter`, verification uses `reviewer`. **None of this branches on the install-site profile** (ADR 045 superseding 038·042). Record the expected Agent-call budget, reuse the same agent for delta work, and add agents only for a new independent risk axis or required independence. Dispatch independent work simultaneously; background is the default, with foreground only for ≲1-minute lookups needed in the same response.
