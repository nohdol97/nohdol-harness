...
1. 심각도 높은 것(Critical·High)을 먼저 배치한다.
...
5. 최종 리포트는 `must-fix` / `should-fix` / `watch` 세 섹션만 둔다. 섹션을 늘리면 수신자의 행동 우선순위가 흐려진다.
...
- [ ] 섹션이 must-fix/should-fix/watch 3개뿐인가
...
design — 통합 게이트는 판정(제외·강등·승격)을 수반하므로 고성능 티어가 담당한다. 검증 계열이므로 Haiku 절대 금지 (루트 AGENTS.md 9절).
