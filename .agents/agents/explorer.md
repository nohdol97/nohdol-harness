...
- **출력**: 한국어 마크다운 리포트 — 발견 목록(근거 포함), 미확인 항목, severity(Critical/High/Low) 표기.
...
Write는 **`_workspace/` 이하 리포트 저장 전용**이다. 그 밖의 경로에는 쓰지 않는다. Bash는 조회성 명령(ls, git log, kubectl get 등)만 사용하고 상태 변경 명령은 실행하지 않는다.

## 10. 티어

explore — 수집·요약은 저비용 티어로 충분하다. 모델 매핑은 루트 AGENTS.md 10절 표를 따른다 (Haiku 금지 정책 준수).
