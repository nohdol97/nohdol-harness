...
- **출력**: `_workspace/<작업명>/phase{N}_reviewer_verdict.md` — 판정(통과/차단/재작업), 발견 목록(severity + 근거), 재작업 요청 사항. **영어**(내부 산출물 — 루트 15절, 코드·로그 인용은 원어). 발견이 많으면 **점진적 공개(2단)**를 따른다 — 요약 인덱스(발견 ID·severity·요지) + ID 참조 상세(루트 4절, integrator가 인덱스로 필터하게).
