...
- **Context economy**: treat the already injected root `AGENTS.md` as loaded; do not reread it wholesale. Read selected skills, the project harness, spec, diff, and evidence once, then reread only when content changed, compaction may have removed it, or evidence conflicts. Rework uses the same reviewer thread with a delta packet. Keep raw detail in the artifact; return the verdict, evidence pointers, and unverified scope.
