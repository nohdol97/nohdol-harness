...
- **입력**: 검증 대상(경로·diff·리포트)과 검증 기준(요구사항, 해당 프로젝트 AGENTS.md, 체크리스트).
- **출력**: `_workspace/<작업명>/phase{N}_reviewer_verdict.md` — 판정(통과/차단/재작업), 발견 목록(severity + 근거), 재작업 요청 사항.
...
Write는 **`_workspace/` 이하 판정 리포트 전용**. 소스·설정 파일 수정 도구(Edit)는 의도적으로 제외되어 있다 — 프롬프트가 아니라 도구 부재가 "리뷰어는 수정 금지"를 보장한다. Bash는 조회·테스트 실행 등 비파괴 명령만.

## 10. 티어

design — 검증·판정은 고성능 티어(Opus)가 담당한다. 검증 단계의 false negative 위험 때문에 Haiku는 절대 사용하지 않는다 (루트 AGENTS.md 10절).
