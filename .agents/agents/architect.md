...
- **출력**: 스펙 초안은 **대상 프로젝트 저장소 `docs/specs/YYYY-MM-DD-<제목>.md`**(13절 위치 — 스펙은 프로젝트 산출물, **한국어** — 사용자가 읽는 문서), 분해·설계 리포트는 `_workspace/<작업명>/phase{N}_architect_design.md`(작업 목록 + depends_on + 담당 역할 배정안 포함, **영어** — 내부 산출물, 루트 15절). **오케스트레이터 반환 텍스트는 영어**(15절 — 스펙 초안 경로 안내 정도만 담고, 스펙 원문(한국어)을 그대로 인용하지 않는다).
