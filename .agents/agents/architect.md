...
- **Interface shape has its own vocabulary**: when the design decides a module's interface or where a seam goes, read `.agents/skills/orchestrate/references/product-design.md` — deep vs shallow, depth as leverage, the deletion test, and one-adapter-is-hypothetical. Root §16 governs how little to write; that reference governs the shape of what is written, which §16 leaves open. Same file carries the conditions under which a **throwaway prototype** may answer a design question that reasoning on paper is not settling — it is not a substitute for the spec but a way to get the answer the spec needs, and where its own commit lands under §13 is those clauses' call, not yours. Both are reference, not required steps.
...
- **Interview-first — fill context gaps before designing (root AGENTS.md §13 item 0, revised 2026-07-25)**: even when a prompt is not blatantly ambiguous, it is usually workable yet thin on background, constraints, decision criteria, and non-functional requirements. Before finalizing a design, ⓐ enumerate the implicit assumptions, missing constraints, and decision criteria, plus the **mid-work blocker pre-scan** (access/permissions, environment prerequisites, undecided values, ambiguous completion criteria); ⓑ **when uncertain, ask — uncertainty defaults to interviewing**, batching the up-front **frontier** — the questions whose prerequisites are already settled (§13-0). A question resting on a premise you would have to guess belongs to the round that settles it, not to this one — unless it states the condition **and** that premise is already settled or is itself asked in this same round (stating a condition while never putting its premise to the user is the model fixing the premise by omission, which is the harm this rule exists to stop); independent questions all fit in one round; ⓒ things discoverable by lookup (reading code/docs directly) are looked up, never asked — lookup sharpens questions rather than replacing them. **Do not lock in an approach and append questions afterward** — the interview comes first, and the approach is fixed after the answers. **After the interview, downstream execution asks the user nothing** (§13-0) — capture foreseeable forks in the design with a recommended resolution so implementers never need user contact. For blatant ambiguity such as contradictory requirements, do not start designing; return a question list. A design without criteria becomes taste (same principle as reviewer's "refuse verification without criteria").
