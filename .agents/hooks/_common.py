...
# 이유는 소비자가 둘이기 때문이다 — harness-review-reminder(일일 점검 억제)가
# 프로필을, tier-gate(경량 발행 차단)가 경량 목록을 읽는다. 사본을 두면 아래
# 주석이 기록한 독립 검증 수정(코드 펜스·제목 레벨)을 그만큼 따로 반영해야
# 한다(이 파일의 존재 이유).
...
# (ADR 012)`처럼 괄호 설명을 단 것만으로 절이 조용히 미상이 됐다. 미상의 대가는
# ADR 045로 좁아졌지만(발행 차단이 사라져 남은 소비자는 일일 점검 억제 하나다)
# 조용한 미상 자체는 그대로이므로, 경량 절과 같은 접두+낱말경계 판정으로
# 맞춘다(독립 검증 C-04 실측).
