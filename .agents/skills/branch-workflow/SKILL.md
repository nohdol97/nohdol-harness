...
description: "Start and finish subproject feature branches: sync main, branch, rebase, push, and PR. Use for 작업 시작, 브랜치, PR 생성, 머지 준비, ship, push to main. Subprojects only; never auto-merge; never push directly to main. Re-run: branch, PR, rebase, 브랜치, 작업 시작."
---
...
4. **Create the PR**: `gh pr create --repo <owner/repo>` — **`--repo` for the same reason as in the cleanup section** (cwd is the harness root, so an unqualified `gh` targets the root harness repository); derive it from `git -C project/<name> remote get-url origin`. Title in commit-convention format; body follows the **doc-writer PR template** (`.agents/skills/doc-writer/references/templates.md` #5: 요약/변경 내용/연결/검증/리뷰 포인트) exactly. If the task is registered in work-tracker, put `Closes #<issue-number>` in the 연결 (links) section (auto-closes the issue on merge). The 검증 (verification) section must contain actual commands and results — no evidence-free "tests pass". **The 변경 내용 section carries the diagram obligation, and this step is where it is held** — a PR body is authored after the review window closes, so no reviewer sees it before it ships (ADR 039). Apply the `diagram` skill's thresholds to what this PR changed; where one is met, draft the body to a scratchpad `.md`, run `check.py` on it, then submit. **A met threshold with no diagram, or a `check.py` failure, blocks PR creation exactly as an absent 독립 검증 line does** — otherwise this clause advises where the line beside it withholds, and "held" would mean two different things inside one step. **It must also carry the template's 독립 검증 line, and this step holds the PR until it does**: either the reviewer rounds and their verdict, or — where the install-site profile exempted that dispatch (`사내`, ADR 038) — the exemption stated in those words plus what was verified in its place. **An absent line blocks PR creation exactly as an evidence-free 검증 section does**, because it is the only step where the exemption becomes visible to the person merging; without it an exempted PR and a silently unreviewed one are the same document. Report the PR URL.
5. **The user does the merge.** After merge, clean up per the section below — the start procedure presumes "branches and worktrees are disposable".
