...
4. **Comprehension quiz gate (root AGENTS.md §13 first-class principle — user comprehension is a completion requirement; this step is the quiz spec single source)**: a **hard delivery gate, not a formality** — a failed or **skipped** comprehension check blocks the PR exactly as a failing test blocks §13-2 delivery (do not open the PR without it).
   ...
   - **Scope exception**: this quiz step alone also applies to **two root paths**, even though the rest of this skill is subproject-only — root remote-PR sessions (root AGENTS.md §5 cites it), and **root commits straight to main when the change is normative** (§13). On that second path there is no PR, so the held step is `git push`: run the quiz after the commit and before pushing, and a wrong or skipped answer holds the push exactly as it holds a PR here. Everything else — question shape, count, the diff reading-guide, the open-questions list — is unchanged, which is why this is a scope line rather than a second spec.
