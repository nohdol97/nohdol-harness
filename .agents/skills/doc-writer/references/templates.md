...
- 독립 검증: <reviewer 라운드 수와 판정(PASS/BLOCK·반영 내용). **어떤 이유로든 발행 없이 갔으면 「독립 검증 없음」이라고 적고 그 자리를 무엇으로 메웠는지**(직접 재실행한 명령·확인 범위)를 잇는다. **이 줄은 비워 두거나 생략할 수 없다** — 줄이 없으면 검증한 PR과 조용히 안 한 PR이 같은 문서가 된다. 설치처 프로필로 갈리지 않는다(ADR 045)>
