...
4. **Background dispatch**: dispatch defaults to `run_in_background` even when the verdict gates the next step — end the turn and resume from the completion notification (CLAUDE.md anchor, user decision 2026-07-24; the session stays conversable during the review — the Phase 0 review-window quiz rides this window).
