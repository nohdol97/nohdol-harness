...
import { Coordination, HandoffStrip, TechnicalDetails } from "../src/visualizations";
...
describe("simplified operator workspace", () => {
  ...
  it("renders the detail without an execution timeline", async () => {
    const detailed = { ...task, agents: [{ id: "agent-1", task_id: "T1", role: "implementer", status: "running", started_at: "2026-08-19T10:00:00", finished_at: "2026-08-19T10:10:00" }], dispatches: [{ wave: 1, task_ids: ["T1"] }], events: [{ ts: "2026-08-19T10:00:00", event: "task_dispatch", task_id: "T1" }] };
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, json: async () => detailed }));
    ...
    const openHeadings = Array.from(view.container.querySelectorAll("h3")).filter((node) => !node.closest(".technical-details")).map((node) => node.textContent);
    expect(openHeadings).toEqual(["T 핸드오프", "실행 조율"]);
    expect(view.container.querySelector(".agent-track")).toBeNull();
    expect(view.container.querySelector(".detail")?.textContent).not.toContain("시간축");
    fireEvent.click(view.container.querySelector(".technical-details summary")!);
    expect(within(view.container.querySelector<HTMLElement>(".technical-details")!).getByText("8월 19일 오전 10:00 → 8월 19일 오전 10:10")).toBeVisible();
    view.unmount();
  });
