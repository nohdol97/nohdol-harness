---
name: harness-review
description: "Review harness operations: daily expansion signals or weekly efficiency/integrity, including harness-native usage; propose metaskill actions. Use for reminders, 일일 점검, 주간 점검, 하네스 리뷰, 진화. Not for applying changes (metaskill) or one installed external tool (tool-audit). Re-run: harness-review, daily, weekly, evolve, 일일 점검, 주간 점검, 진화."
---
...
The cadence column describes the **automatic** trigger. On a `사내` install profile the reminder hook does not fire daily mode at all (spec `2026-07-14-harness-review-reminder-hook` R8) — findings there cannot be applied, since that site may not edit tracked harness files (root §5). Weekly is unaffected, and an explicit user request runs either mode on any profile.
...
| ④ Shrink/efficiency (weekly full only) | **3+ weeks uninvoked**: search each skill/agent name with `agentsview session search` to check recent invocation history — **two paths, and a tally of `Skill` tool calls is neither**. A slash invocation records a *user message* and produces **zero** `Skill` tool calls, so a tool-call tally reports an actively used skill as dead; an unanchored name search fails in the opposite direction, since every skill name also appears in `README.md`, the changelog, and edit bodies, which makes "uninvoked" never-true and silently disables the retirement half of this signal. So query **both**: the anchored user invocation (`agentsview session search '^/<name>$' --regex --in messages`) and the routed one (`Skill` calls, for skills reached by routing rather than typed). Both mechanisms measured 2026-08-02; no count is quoted here because this clause names the skills it searches and would match itself. If neither path shows a use for 3+ weeks, propose retirement/consolidation (if agentsview is not installed, withhold judgment — no retirement proposals without evidence). **Token-overconsumption pattern 2+**: check top token/cost sessions via `agentsview stats`, and confirm via session logs whether team formation for simple tasks, repeated collection of the same information, or unnecessary full-file loading recurred → propose procedure improvements (downgrade the judgment, reuse artifacts, split into references/) |
...
**Machine judgment first (integrity-check.py)**: Symlinks (R1), `.claude/` real-file intrusion (R2), skill/agent frontmatter (R3·R4), MOC consistency (R5), CLAUDE.md first line (R6), gitignore entries (R7), Codex adapters/config (R12·R18), AGENTS.md size (R14), ADR references (R15), and Korean-view drift (R16·R17) are **judged deterministically by `python3 .agents/hooks/integrity-check.py`** (spec: docs/specs/2026-07-19-integrity-check-script.md). **Run it first** and digest its PASS/FAIL/SKIP into the report — do not re-read those structures. Installation sites where symlinks are impossible show R1·R2 as SKIP. Only semantic items the script cannot cover (git hook registration, `_common.py` behavior, cross-CLI hook meaning, sub-harness/registry consistency, leftovers) are checked directly below.
...
- Codex hook registration contract: consume integrity-check R18, which verifies `.codex/config.toml` uses the canonical `hooks` feature, 64KiB document headroom, inline SessionStart·PreToolUse·PostToolUse definitions, and no parallel `.codex/hooks.json`. Semantic parity with `.claude/settings.json` remains a reviewer judgment because the CLIs have different event schemas (ADR 031).
...
- **AGENTS.md Codex loading budget**: consume integrity-check R14; more than **32,000 bytes is FAIL**, leaving safety margin under Codex's default 32,768-byte project-document limit. Diet technique: keep always-on rule+short reason, move skill-specific procedure and case narratives to their existing single sources.
...
- **Ops log**: append a one-line review result to `_workspace/harness-ops-log.md` — format: `- YYYY-MM-DD [전체 점검|일일 점검] 신호 N건(요약) / 무결성 통과|문제 N건 / 제안 N건(요약)` (Korean — this log is user-read). This log is the single out-of-chat record of "what the last review saw and what was improved" (improvement executions are appended by metaskill as `[개선]` items). Since the reminder hook stays silent before the due date (noise removal, 2026-07-16), past history is checked directly through this log. **Re-run the integrity check immediately before writing this line and record that run's value, not the one from review start** — this skill has the main loop working in parallel by design (execution-method section above), so tracked files can change mid-review and a verdict formed at the start may no longer reproduce. When the two runs differ, record **both** plus `git status --porcelain` naming what moved in between; a figure that no longer reproduces is exactly what metaskill common rule 14 forbids.
