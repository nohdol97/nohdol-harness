...
- Worktree: <absolute path, when the work runs in one — omit the line otherwise; the branch goes on the line below so the two cannot disagree>
- Branch: <the branch the work is on — **always written when the target is a subproject**, worktree or not. On a `사내` profile there is no worktree (ADR 043) and the branch lives in the shared `project/<name>/` checkout, which any other session can move off; this line is the only thing that tells the resuming session which branch its work was on>
...
4. **Re-entry**: continue along the path the note's "How to resume" points to. **If the note carries a Worktree line, resume at that path** — verify it with `git -C project/<name> worktree list` and never create a second worktree for the same branch (`branch-workflow` start procedure, edge cases); a resume that starts a fresh branch in `project/<name>/` splits the work across two directories. **With a Branch line and no Worktree line** (the `사내` shape — ADR 043), **read the checkout's current branch before touching anything**: `git -C project/<name> symbolic-ref --short HEAD`, and if it does not match the note, switch to the note's branch rather than working where you landed. That checkout is shared and another session moves it, so a resume can arrive on `main` — the silent failure ADR 035 recorded and that worktrees made structurally impossible. This re-check is `branch-workflow`'s fork step 3, restated here because **the resuming session loads this skill and not that one**. If it is new implementation or behavior change, **step through the orchestrate gate again** (§7 item 3; continuation also re-enters the gate). The gate **also rules on the implementation owner** (3+ files → dispatch implementer; high risk → team ruling — orchestrate implementation-owner rule): a session heading straight into direct implementation right after resume, without a gate ruling, is a bypass signal (real case 2026-07-21). Review → team-review; documents → doc-writer. carryover itself performs no work — it only hands over the re-entry point.
