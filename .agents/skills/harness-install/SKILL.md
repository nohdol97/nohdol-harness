...
### 3. Auxiliary tool installation (all optional — the install procedure continues even on failure)

#### 3a. agentsview (recommended)
...
#### 3b. defuddle (optional — web content extraction)

The CLI used by the `defuddle` skill. It extracts only the main content from web pages, reducing tokens versus WebFetch (removes nav/ads/sidebars). **If this installation site frequently does web research/document collection (deep-research (Claude Code built-in)·doc-writer)**, install globally with `npm i -g defuddle`.

- **Not required**: even uninstalled, the `defuddle` skill automatically falls back to WebFetch, so skipping does not kill functionality — it is pure token optimization.
- Check installation: `command -v defuddle`. If offline, lacking global npm permission, or refused by the user, skip and note it in the completion report.
...
#### 3c. context7 MCP (optional — library documentation lookup)

The MCP server used by the `context7` skill. It queries **version-specific, up-to-date documentation** for libraries/frameworks/SDKs in real time, eliminating memory-based wrong answers (deprecated APIs, changed configuration). **If this installation site does third-party-library development (web/app/backend)**, register it.

- **Not required**: even unregistered, the `context7` skill automatically falls back to WebFetch/WebSearch, so skipping does not kill functionality — pure accuracy/token optimization (same nature as defuddle).
...
- If offline, blocked by network policy, or refused by the user, skip and note it in the completion report. **Never register without approval.**
...
#### 3d. Claude Code global recommended settings (optional)

Global settings that help session observability/retention. Put all of them in **`~/.claude/settings.json` (per-machine)** — they are personal display/retention preferences, not the tracked harness `settings.json` (policy) (same layer as hooksPath·global MCP registration). For each item, **ask the user whether to enable it** (optional), preserve existing keys, and validate the JSON by parsing after applying. On refusal, skip and note it in the completion report (leaving all unset is harmless).
...
**② Transcript retention extension (cleanupPeriodDays)** — The auto-deletion period for session transcripts. If the default **30 days** feels short, propose an extension (better for audits/tracing past sessions; costs only a little more disk). In `settings.json`: `"cleanupPeriodDays": <days>` (e.g. 90–365). Confirm the period with the user and apply.
...
#### 3e. rtk (optional — command output token compression proxy)

A Rust CLI where `rtk <cmd>` runs the command on your behalf and compresses the output (e.g. git status ~3,000→~600 tokens), reducing context consumption (Apache-2.0, adoption design: docs/proposals/2026-07-22-rtk-adoption.md). Its surface overlaps with this workspace, where kubectl·log·git·test output is heavy. **Not required** — nothing dies without it.

1. Install (global — commit nothing to the workspace): macOS `brew install rtk` (core formula — README-verified path); Linux via GitHub Releases binaries or `cargo install --git https://github.com/rtk-ai/rtk` (beware the same-named unrelated package on crates.io — the README warning is real). Proceed after user confirmation; **no unauthorized installs**.
...
- [ ] agentsview installed + local SQLite DB (`~/.agentsview/` or `AGENTSVIEW_DATA_DIR`) creation confirmed + external DB sync unset + finding-history skill installed globally (if skipped, the reason is in the completion report)
- [ ] (Optional) defuddle installed, or the skip reason is in the completion report — absence is harmless (the `defuddle` skill falls back to WebFetch)
- [ ] (Optional) context7 MCP present in `claude mcp list` (user scope), or the skip reason is in the completion report — absence is harmless (the `context7` skill falls back to WebFetch/WebSearch). If SuperClaude leftovers magic·sequential-thinking exist, removal was offered
- [ ] (Optional) Claude Code global recommended settings (3d) explained — ① always-on context display (`statusLine` in `~/.claude/settings.json` + `~/.claude/statusline.py`), ② transcript retention extension (`cleanupPeriodDays`). Each item applied, or the user refusal/skip reason is in the completion report. For Codex, it was explained that context display is on by default and retention is indefinite by default (neither needs configuring)
