...
#### 3g. Understand-Anything — code knowledge-graph plugin (default)

A **direct-invocation-only** onboarding/navigation aid (MIT, Egonex-AI/Understand-Anything): a multi-agent pipeline builds a per-project knowledge graph (`.ua/knowledge-graph.json`) with an interactive dashboard, guided tours, fuzzy/semantic search, and diff-impact view. Adopted for **human-facing onboarding into unfamiliar/large codebases** — adoption design and boundaries: docs/proposals/2026-07-25-understand-anything-adoption.md.

1. Install (global — nothing enters the tracked repo, §11):
   - **Claude Code**: native plugin — the two commands are interactive slash commands the **user runs**: `/plugin marketplace add Egonex-AI/Understand-Anything` then `/plugin install understand-anything` (a Claude session cannot run `/plugin` via Bash — guide the user, like `gh auth login`). **Corporate/marketplace-blocked note**: on Claude Code, Understand-Anything is native-plugin-only (the repo's `install.sh` covers non-Claude CLIs, not Claude Code) — so if a 사내 network/policy blocks the plugin marketplace, there is **no documented marketplace-free path → skip with reason** (absence harmless, direct-invocation convenience). On non-Claude CLIs, `install.sh` does not use the marketplace.
   - **Other CLIs (Codex/OpenCode/Gemini CLI/…)**: `curl -fsSL https://raw.githubusercontent.com/Egonex-AI/Understand-Anything/main/install.sh | bash -s <platform>` (global clone to `~/.understand-anything/repo`). Proceed after user confirmation.
   - **§3 runtime caveat (state it at install)**: `/understand`'s semantic-search step sends code content to the configured **embedding provider**; a default *external* provider (OpenAI/Gemini/…) egresses code to a third party. Scope by profile: **사내 (corporate) profile or any proprietary codebase → enable a local embedding model first** (`all-MiniLM-L6-v2` via `@xenova/transformers` — lightweight, CPU-only, no GPU or strong machine needed), so nothing leaves the machine. **개인 (personal) profile on your own/open code → not needed**; the default external embedding is acceptable. This is a usage caveat, not an install gate (default-install holds). A local *LLM* (Ollama) is a separate, heavier, **optional** posture (avoids egress to the analysis model itself) — not required for §3.
   - **Sub-project gitignore (state it when a sub-project first runs `/understand`)**: the per-project graph lands under `.ua/` — the root `project/` is gitignored, but sub-projects are independent repos, so add `.ua/` to that sub-project's `.gitignore` (a generated, machine-local artifact).
2. **Direct-invocation-only routing (mandatory — the collision guard)**: the plugin's `understand-*` skills are **never auto-routed** — they run only on the user's explicit `/understand…` (Codex: `$understand`). This is a harness routing policy (we do not edit the external plugin's global frontmatter, §11), so state it in the completion report and it is enforced by §7 precedence + the CLAUDE.md routing note. Reason: keeps token cost opt-in-per-use (README warns `/understand` is token-heavy on large repos) and prevents trigger collision with explorer/orchestrate.
...
- [ ] Understand-Anything installed (default-install; Claude Code = user runs the two `/plugin` commands, other CLIs = install.sh) + direct-invocation-only routing stated + graph-not-ground-truth scope boundary stated; on a marketplace-blocked corporate machine Claude Code has no marketplace-free path → skip with reason (absence harmless); if skipped, the reason is in the completion report
