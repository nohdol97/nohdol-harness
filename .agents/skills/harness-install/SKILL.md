...
1. Install (global — nothing enters the tracked repo, §11):
   - **Claude Code**: native plugin — the two commands are interactive slash commands the **user runs**: `/plugin marketplace add Egonex-AI/Understand-Anything` then `/plugin install understand-anything` (a Claude session cannot run `/plugin` via Bash — guide the user, like `gh auth login`).
