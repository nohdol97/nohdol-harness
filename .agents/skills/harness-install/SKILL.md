...
1. Install (global — nothing enters the tracked repo, §11):
   ...
   - **§3 runtime caveat (state it at install)**: `/understand` with default external embedding/model providers **egresses code content** to that provider. On a **사내 (corporate) profile, or for any proprietary codebase, configure a local model (Ollama) before running `/understand`** — this is a usage caveat, not an install gate (default-install still holds), but running it against external providers on protected code violates §3.
