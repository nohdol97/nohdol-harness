...
| **생성-검증 쌍 (구현 최소 단위)** | 기능 추가·동작 변경(13절 대상)인데 다중 관점 팀까지는 불필요 | **3+파일 또는 중위험 이상: implementer 1 + reviewer 1 — 메인 루프가 직접 구현하지 않는다.** ≤2파일·저위험: 오케스트레이터 직접 구현 허용 + 독립 reviewer 필수(경량 리뷰 프로토콜 — 자기 검증 금지는 불변). 아래 "검증 필수 규칙"·"구현 주체 규칙" |
...
- **구현 주체 규칙 (하위 프로젝트 제품 코드)**: 위 쌍 판정에서 implementer 발행을 "맥락 전달 비용이 크니 내가 직접 하는 게 싸다"로 대체하지 않는다 — 그 정당화 자체가 우회 신호다(2026-07-21 실측: 하위 프로젝트·재개 세션에서 메인 루프 직접 구현 패턴 반복). **≤2파일·저위험만 직접 구현이 공식 경로**이고 이때도 reviewer 독립 검증은 생략 불가(자기 검증 금지가 13절 보증의 본질). 직접 구현하던 작업이 3+파일로 자라면 그 시점에 implementer로 넘긴다(전환 신호). 이유: 메인 루프 직접 구현은 컨텍스트를 구현 디테일로 비대하게 만들고, 전체 진행을 보는 주체를 없앤다(리더 역할 절과 동조).
...
**소규모 diff의 검증 발행은 경량 리뷰 프로토콜을 따른다**(단일 원본: team-review "단독 모드" 절 — 2026-07-21 실측, 소규모에서도 리뷰가 수 분 소요): 오케스트레이터는 reviewer 발행 프롬프트에 **diff 전문·스펙 완료 기준 발췌·직전 테스트 출력을 인라인 포함**하고(재수집 왕복 제거 — 관련 테스트 재실행은 리뷰어가 유지), `run_in_background`로 발행하며(CLAUDE.md 앵커), ≤2파일·저위험이면 리포트 파일 대신 반환 텍스트 판정을 받는다(must-fix 발견 시에만 파일화). 검증을 깎는 게 아니라 고정 오버헤드를 깎는 규칙이다 — 이 절의 검증 필수 자체는 불변.
