...
10. **Bound tool and visible output**: for exploratory reads and diffs, prefer installed `rtk` or targeted ranges/filters; completion evidence uses raw or tee-preserved output. User progress updates default to 1–2 sentences. The completion response defaults to result, key evidence, and unverified scope; safety decisions, failure analysis, and the comprehension quiz may expand.
...
**Work that includes feature addition or behavior change includes a final independent reviewer verification phase regardless of the verdict grade** — under a generate-verify pair verdict, the pair's reviewer does it; under a team verdict, the verification phase does (spec-based review per §13 item 3 — the spec's completion criteria are the verdict criteria). Reason: an implementer's self-assessment passes its own bias — if verification is optional, it gets skipped first precisely on busy work. If the scale is large (same as the team threshold in the 0-1 verdict table — 6+ changed files) or the risk is high, promote the verification phase to team-review team mode (perspective fan-out). **Review-window quiz (pair and team verification alike — **sub-project** PR-bound work only; harness changes are exempt on every path including the root remote-PR cycle, §13, user decision 2026-08-02)**: right after dispatching the background verification, run the §13-0 comprehension quiz with the user in the same turn (quiz spec single source: branch-workflow finish 4) — quiz and review in parallel, so review-wait time carries the quiz and misalignment surfaces before PR mechanics; if later rework changes behavior, finish 4 delta re-quizzes.

**Install-site exemption — `사내` profile only (ADR 038, the single source for this branch)**: where REGISTRY.md's 「설치처 프로필」 records `사내`, verification-purpose dispatch is off and the mandate above does not produce an Agent call — `reviewer`, team-review perspective fan-out, `infra-specialist` in review mode, and `integrator` **when it is fanning in review reports** are all included; build-side dispatch is untouched, and so is `integrator` doing ordinary fan-in that is not a review — `project-status` Phase 2 merges status artifacts and is unaffected. **The hook enforces only the `reviewer` type**, because `integrator` and `infra-specialist` each serve a non-review purpose that `subagent_type` cannot distinguish from the review one; those two axes are rule-governed, and a gate that blocked them by type would kill `project-status` on a corporate site with no sanctioned escape. **What replaces it is not nothing**: the orchestrator verifies against the spec's completion criteria itself, records the §13-2 evidence four (command/surface, observed output, why it proves the criteria, unverified scope), and adds **one line stating that independent verification was profile-exempted** — without that line the artifact reads as reviewed, which is the failure this exemption is written to avoid, not a side effect of it. Self-verification is not equivalent and the record must say so. **That record has a step that holds it**, or it is not a requirement: for PR-bound work it is the PR body's 독립 검증 line, which branch-workflow finish 5 refuses to open a PR without; for a harness change it is the change-history row (metaskill improvement mode). Where neither applies, the completion report carries it — but name the holder rather than leaving the obligation floating. **An explicit user review request still dispatches** (§8's shape: automatic firing off, explicit request on) — that dispatch carries `[review-ok]` in the prompt, which is added only after the user asked. Enforced at dispatch by `.agents/hooks/review-gate.py`, whose scope is the `reviewer` type as stated above. **Also read the run-point consequence**: with no review window, the branch-workflow finish-4 quiz runs at finish before PR creation instead of in parallel with a review. Everything else in §13 is unchanged — tests, the spec gate, and §3 guardrails are not what this removes.
...
| ④ Independent verification | Generate-verify | Subagent (reviewer — isolation is the independence) | **Cannot be skipped when implementation is included** — on a `사내` profile it becomes main-loop self-verification plus a recorded exemption (Verification-mandatory rule) |
