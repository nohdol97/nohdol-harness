...
5. **Interview-first (all work — root AGENTS.md §13 item 0, revised 2026-07-25)**: if the verdict is **team** and an architect is placed, the architect performs the interview. When the verdict is **handle directly / generate-verify pair** — no architect involved — the orchestrator runs it itself before starting: enumerate implicit assumptions, missing constraints, and judgment criteria, plus the **mid-work blocker pre-scan** (access/permissions, environment prerequisites, undecided values, ambiguous completion criteria); **when uncertain, ask (AskUserQuestion) — uncertainty defaults to interviewing**, all questions batched into this single up-front round; lookup-able things are looked up, never asked. **Once execution starts, no questions to the user** — resolve forks with the best-recommended option and surface each decision in the completion report and the pre-PR quiz (§13-0 carve-outs for §3 and data-loss confirmations apply). Reason: locking in an approach and appending questions afterward incurs the loss of precisely solving the wrong problem, and front-loading keeps execution unattended-safe (architect embeds this procedure; the direct and generate-verify paths need an owner — this step).
