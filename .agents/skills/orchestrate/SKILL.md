...
- **Agent-call budget (record at Phase 0)**: state the expected number and roles of Agent calls before dispatch. Defaults are answer/lookup=0, isolated collection=1, ≤2-file low-risk direct implementation=1 reviewer, 3+-file generate-verify=1 implementer + 1 reviewer, and a team phase=only the applicable 3–5 workers (plus integrator when fan-in is real). Allocate **mandatory verification first**, then add agents only for independent work whose context isolation or parallelism pays for another model/tool run. Reuse the existing agent thread for the same role and perspective; send only the changed diff, tests, and decisions for rework or re-verification. Spawn another agent only when a **new independent risk axis** appears, the prior thread is unavailable, or independence from the author requires a different owner. This is an expected budget, not a hard cap: safety, security, infra, and spec verification remain mandatory. Reason: each extra agent performs a complete model/tool run, while a delta follow-up preserves the prior criteria without rebuilding its context.
