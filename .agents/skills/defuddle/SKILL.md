...
4. **원격/샌드박스 환경 주의 (이 하네스가 흔히 도는 환경)**: 아웃바운드 HTTPS가 에이전트 프록시를 경유한다. defuddle이 TLS 검증 실패·403/407을 내면 프록시를 안 타는 것이므로, **TLS를 끄거나 `HTTPS_PROXY`를 해제하지 말고**(루트 환경 가드레일) 그 URL은 **WebFetch로 폴백**한다 — WebFetch는 하네스 도구라 프록시가 이미 처리된다. defuddle을 프록시에 맞추는 설정 튜닝에 시간을 쓰지 않는다.

5. **결과 사용**: 추출된 마크다운을 그대로 요약·인용의 입력으로 쓴다. deep-research·doc-writer가 URL을 읽어야 할 때 이 스킬이 1차 경로다.
