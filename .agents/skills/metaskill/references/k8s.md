...
- 초기에는 **단일 에이전트 + 조회 전용 tools**로 시작 (완벽주의 금지).
- 유용한 초기 스킬 후보: 클러스터 상태 요약, 매니페스트 diff 리뷰(생성-검증 패턴), 배포 전 체크리스트.
- 에이전트 tools 화이트리스트에서 변경 계열 명령은 기본 제외하고, 필요 시 개별 에이전트에만 부여한다 (tools가 가드레일 1순위).

## 연관 프로젝트 라우팅

- 웹/앱 배포 요청은 대부분 크로스 프로젝트다 (예: web 빌드 → k8s 매니페스트 → gitops 반영). 레지스트리 "연관 프로젝트" 컬럼에 반드시 상호 기재한다.
