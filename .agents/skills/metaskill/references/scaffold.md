...
**Then mirror the finished sub AGENTS.md to a secret gist and record it**, because the file rides in no repository (ADR 005·006) and the root harness repo may be public — check with `gh repo view --json visibility` before assuming otherwise. `gh gist create -d '<name> 하위 하네스 (사내 반입용)' .agents/projects/<name>/AGENTS.md` (gists are secret by default; there is no `--secret` flag), then add a row to the REGISTRY.md carry-in transfer table with the gist id. Give the file its own section carrying the edit and fetch commands, so the session that edits it sees them: fetch **must** pass `-f <filename>` — bare `--raw` prepends the description and a blank line, silently adding two junk lines to a markdown file. **The mirror is a copy, so it is worth what its freshness is worth**: editing a mirrored sub AGENTS.md obliges updating the gist in the same task, exactly like its change-history row. Nothing verifies this — the gist is remote, `integrity-check.py` is local — so the working check is comparing the last change-history row across the two machines.
