...
## 12. 하위 프로젝트 하네스 중앙 관리 (ADR 005)

하위 프로젝트 하네스는 이 루트 저장소가 **원본을 추적하고, 하위 프로젝트에는 심링크로 배포**한다. 이유: 스킬·훅·설정은 세션 시작 디렉토리(cwd) 기준으로만 로드되어 루트 세션에서 하위 것이 로드되지 않고, 하위 프로젝트 저장소에는 하네스 파일을 커밋하지 않기로 했기 때문이다(2026-07-12 사용자 확정). 단, gitignore만 하면 하네스가 어디에도 추적되지 않아 유실된다 — 원본을 여기서 추적하는 것이 그 결함의 해법이다.

- **규칙의 운반체는 문서다**: 가드레일·필수 규칙은 반드시 AGENTS.md(문서)에 싣는다. 스킬·훅은 편의 장치일 뿐 규칙의 유일한 운반체가 될 수 없다 — cwd에 따라 로드가 깨지기 때문이다.
- **원본 위치**: `.agents/projects/<이름>/` (AGENTS.md, CLAUDE.md, adr/) — 루트 저장소가 추적한다. REGISTRY.md처럼 설치 환경별 데이터지만 추적한다(ADR 004 선례: 버전 관리 밖의 하네스는 이력·복구 불가).
- **배포(머티리얼라이즈)**: `project/<이름>/`에 AGENTS.md·CLAUDE.md 심링크 + `.claude/agents`·`.claude/skills` → **루트** `.agents/agents`·`.agents/skills` 심링크. 하위 프로젝트 전용 `.agents/`는 만들지 않는다.
- **하위 저장소 보호**: 하위 프로젝트에 git 저장소가 있으면 배포된 심링크들을 그 저장소의 `.git/info/exclude`에 등록한다(저장소 파일을 건드리지 않는 로컬 제외). 하네스는 하위 프로젝트 git에 절대 올라가지 않는다.
- **하위 전용 스킬·에이전트**: 하위 디렉토리가 아니라 루트 `.agents/skills/`·`.agents/agents/`에 `<프로젝트>-` 접두어로 생성한다(예: `web-deploy`). 루트 세션·하위 세션 어디서나 보인다.
- **훅**: 하위 프로젝트 전용 훅은 루트 `.claude/settings.json`에 경로 분기형으로만 둔다(스크립트가 대상 경로 `project/<이름>/`을 검사해 분기). 훅은 세션 루트 것만 실행되기 때문이다.
- **세션 시작 관행**: 단일 프로젝트 작업은 `project/<이름>/`에서, 크로스 프로젝트·하네스 작업은 루트에서 세션을 시작한다.
- **한계(수용한 트레이드오프)**: 하위 저장소만 단독 클론한 환경(원격 세션 등)에는 하네스가 없다. 하위 프로젝트 작업은 이 워크스페이스 안에서 수행하는 것을 전제로 한다.
...
| 2026-07-12 | 12절 신설 — 하위 프로젝트 하네스 중앙 관리(원본 `.agents/projects/`, 심링크 배포, `.git/info/exclude` 로컬 제외, 스킬·훅 루트 단일 소스) | 12절, metaskill, orchestrate, README | 사용자 확정 — 스킬·에이전트는 루트에서 관리하고 하위 프로젝트 git에는 하네스를 올리지 않는다. gitignore만으로는 원본이 미추적되는 결함을 루트 추적으로 보완. docs/adr/005 |
