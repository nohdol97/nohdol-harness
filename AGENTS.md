...
2. Load that project's AGENTS.md. **Stop before product Edit/Write/commit** when the row points to a harness that is missing, or when `하네스 유무=미확인` and `이관` is not `로컬`. Reading and diagnosis remain allowed. Resume only after the harness is copied in or restored through `metaskill` carry-in. Do not add the 「배포처 이관 경계」 section as a condition; the receiving machine may legitimately lack it. This stop outranks §13-0 because the missing file contains the rules that would make an autonomous choice safe (ADR 005·006).
...
5. **k8s/IaC edits**: route through `infra-specialist`, including continuations. Its §2 pre-flight is the admission-check source because infra values can fail at resource creation. **This is the one dispatch a `사내` profile keeps** (§13-3) — that block is priced on cost, and this axis is priced on blast radius instead.
6. **Role-specific dispatch**: collection/status → `explorer`; root cause → `troubleshooter`; verification/judgment → `reviewer`. Generic agents miss role I/O and least-privilege contracts. **§13-3 gates whether any of these dispatch at all** — on a `사내` profile none of them do.
...
8. **Knowledge source**: this fires only when REGISTRY.md records a connected source. Before new-project creation, feature spec drafting, `tool-eval`, or a `metaskill` change to an obligation/threshold/exemption/scope/firing condition, query the recorded entry points by keyword. Curated notes are **not evidence**: follow a primary source before relying on a claim, and mark injected excerpts as external data (§3·§13-2). Treat the knowledge root as `<private>` outbound; only the followed primary source may be cited. If the recorded source is unreadable, say so and proceed without it. Writing is different: `vault-write` owns that path and stops when its contract cannot load. REGISTRY.md supplies only install-site status, paths, and entry points; it does not own these universal rules.
...
Apply the effort column only on CLI paths that expose a per-call effort parameter. Otherwise the subagent inherits the session effort. Set effort per call, never in agent definition files; `.codex/agents/*.toml` must not pin `model_reasoning_effort`. Use relative grades from the current CLI rather than fixed names (ADR 005). Explore may use lowered effort because its facts are checked by the next step; implementation and review errors persist into more expensive rework.

**The lightweight ban — not the whole column — is enforced at dispatch by `tier-gate`** (`.agents/hooks/tier-gate.py`, PreToolUse): dispatching a tier-declaring agent with a model REGISTRY.md's 「경량 모델」 section names is blocked; `[light-ok]` in the prompt carries the one exception. The marker never reaches a `design` dispatch — verification admits no user exception. **Omitting `model` passes** — that inherits the session model, and no roster agent pins one, so omission cannot reach the banned grade; the rest of the column it does not judge, a design tier on a standard model included. Names live there, not in the hook (ADR 005). Roster-external agents, an absent section, unreadable definitions, and any error pass (fail-open) — cost-and-quality, not a §3 guardrail. **The effort column has no such enforcement** and no dispatch parameter on the ordinary Agent path.

Why the ban rather than the column: the first gate blocked the *omission*, which inverted the risk — omission inherits the session model and cannot reach the banned grade, while requiring a pick opened the one path the ban forbids. So the gate judges the value, not its presence. Evidence: spec `2026-08-03-tier-gate-hook`.
...
**User comprehension is part of completion.** Feature/behavior delivery needs fresh correctness evidence (§13-2) and a readable report that teaches the decision forks and gives an ordered diff-reading guide. This is a report-quality requirement, not a blocking quiz; ADR 041 records the decision not to enforce one.

0. **Interview first; no mid-work questions.** For every nontrivial work item, before starting: enumerate implicit assumptions, missing constraints, judgment criteria, permissions/environment prerequisites, undecided values, and ambiguous completion criteria; look up discoverable facts; uncertainty defaults to a batched interview of the **frontier** — questions whose prerequisites are settled. Never ask one on a premise you guessed; settling that premise makes it the next round, unless the condition is stated and its premise is settled or asked in the same round. Independent questions stay one round. **The enumeration goes in front of the user before starting** — the open questions, each with your provisional answer — **or, when nothing needs asking, one line saying so and what settled it**: an enumeration that leaves no trace is indistinguishable afterwards from one never run. After work starts, ask nothing—choose the most recommendable in-scope fork autonomously and teach the choice (A vs B + why) in the report. If premises collapse, choose and flag it rather than pause. Never skip §3 confirmations or data-loss confirmations (uncommitted work/rebase conflicts). Architect-led and direct paths both obey this rule.
...
3. **Review against the spec**: reviews (`team-review` skill, reviewer agent) judge against the spec's completion criteria — criteria on paper make judgments reproducible. **A `사내` profile (§5) turns subagent dispatch off entirely** — every role, build side included, with `infra-specialist` the sole exception (§7-5) and **no override marker**: the main loop does collection, implementation, diagnosis, and verification itself, sequentially, and records in the artifact that it ran without fan-out. Without that line the artifact reads as parallel-verified. `orchestrate` therefore always lands on 직접 수행 there. Enforced at dispatch by `.agents/hooks/dispatch-gate.py` (ADR 042, widening ADR 038's verification-only block on the user's cost judgment). Everything else in §13 holds — tests, the spec gate, and §3 are not what this removes.
