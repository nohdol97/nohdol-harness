...
- **왜 문서로만 이식하나**: 규칙의 유일한 운반체는 문서다(12절) — 배송 기계는 복제하지 않고 **원칙(이 절) + 리뷰 관점 1개(team-review)**만 이식했다.
