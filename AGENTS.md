...
> 이 파일이 단일 원본(single source of truth)이다. Claude Code에서는 **CLAUDE.md가 첫 줄 `@AGENTS.md`로 이 파일 전문을 세션 시작 시 자동 주입**하고(ADR 021), Codex는 AGENTS.md를 네이티브로 로드한다 — 두 CLI 모두 이 파일이 항상-온이다. CLAUDE.md에는 이 파일 위에 Claude 전용 항상-온 앵커(출력 언어·라우팅 요약)만 얹는다.
...
- **리포트 구조 — 점진적 공개(2단)**: 발견이 많은 `_workspace/` 리포트는 ① **요약 인덱스**(발견 ID·severity·한 줄 요지)를 맨 위에 두고, ② 각 발견의 **상세·근거**(`파일:줄`/명령 출력)는 ID로 참조되는 하위 섹션에 둔다. 이유: 리포트는 쓰기 1회·읽기 N회(integrator·메인 루프 재독)라, 소비자가 인덱스로 필터한 뒤 필요한 ID의 상세만 읽으면 재독 토큰이 곱으로 준다(8절 신호 ④ 수축·효율과 같은 목적 — claude-mem의 점진적 공개 이식, ADR 018). 단, **발견이 소수인 짧은 리포트는 인덱스를 생략**한다 — 규율이 노이즈가 되면 오히려 우회된다(16절 최소주의와 같은 결).
...
- **CLAUDE.md 로딩 역할 (재비대 방지 규율, ADR 021)**: Claude Code는 CLAUDE.md만 자동 로드하고 AGENTS.md는 자동 주입하지 않는다(산문 링크로는 안 됨 — `@경로` 임포트라야 주입). 그래서 CLAUDE.md 첫 줄에 **`@AGENTS.md`**를 두어 단일 원본을 항상-온으로 주입한다. CLAUDE.md 본문에는 **전문 규칙을 복사하지 않고**, 매 턴 최우선으로 지켜야 하는 **Claude 전용 항상-온 앵커(출력 언어·라우팅 요약)만 한 줄씩** 둔다(상세·이유는 AGENTS.md 참조). 변경 이력·감사 로그는 CLAUDE.md에 두지 않는다(`docs/harness-changelog.md`). 이유: CLAUDE.md에 정보가 쌓이면 단일 원본이 둘로 갈라지고, `@AGENTS.md`로 이미 전문이 항상-온이라 복사는 순수 중복이다. Codex는 AGENTS.md를 네이티브 로드하므로 이 임포트와 무관하다(패리티는 저절로 유지).
...
**한국어를 유지한다 (사용자가 읽음 — 사용자 대면 출력은 한국어로 고정)**. 이 규칙은 CLAUDE.md 첫머리에 **항상-온 앵커**로도 얹혀 있어(ADR 021 — 서브에이전트 영어 리포트 직후에도 최종 요약이 영어로 미끄러지지 않게 매 턴 눈앞에 둔다), AGENTS.md `@import`(항상-온)와 이중으로 보장한다. 대상:
...
- **왜 문서로만 이식하나**: 규칙의 유일한 운반체는 문서다(12절). ponytail은 이 원칙을 Node.js 훅·플러그인·6스킬로 배송하지만, 훅·플러그인은 cwd 로드 편의 장치라 규칙의 유일한 운반체가 될 수 없다(12절 — SessionStart 리마인더 훅의 Codex 병행(11절)은 예외이되 그것도 규칙 원본은 문서다). 그래서 배송 기계 전체를 복제하지 않고 **원칙(이 절) + 리뷰 관점 1개(team-review)**만 이식한다 — 이 채택 자체가 사다리 2번(재사용)·5번(기존 의존성)의 실천이다.
