...
**User comprehension is part of completion.** For feature/behavior work, delivery requires both fresh correctness evidence (§13-2) and demonstrated understanding via the branch-workflow comprehension quiz. A failed or skipped quiz blocks the PR like a failed test. Run the primary quiz in the review window alongside the background reviewer; PR finish confirms it and delta re-quizzes behavior-changing rework. Teach decision forks and provide an ordered diff reading-guide; the goal is user growth, not opaque automation. **The root harness has two paths and only one of them carries the quiz**: the remote PR cycle (§5) already runs commit/push → quiz → PR, but a local root session commits straight to main, where there is no PR for the quiz to ride — **there it rides the push**. A **normative** root change (obligation, threshold, exemption, scope, or firing condition — metaskill's boundary) is quizzed after the commit and **before `git push`**, and a wrong or skipped answer holds the push exactly as it holds a PR; non-normative diffs (wording, pointers, history rows) are exempt. **This clause is also the one place §13 reaches a change that is not code** — the section otherwise scopes itself to code work, and the repository where the rules themselves change produces little else. Reason: otherwise the requirement is a declaration on the one repository it most needs to bind.
