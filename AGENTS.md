...
Why the ban rather than the column: the table had never been applied (dispatches inherited the session model), so the first gate blocked the omission itself — inverting the risk. Inheritance cannot reach the banned grade, so requiring a pick opened the one path the ban forbids, and a dispatch took it that day. Enforcement stays at dispatch, but now judges the value: checking only that a value exists let both the banned grade and the design-tier case pass. Evidence: spec `2026-08-03-tier-gate-hook`.
