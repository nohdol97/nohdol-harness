...
- **This always-on file carries rules + rationale (why), not provenance** (extends ADR 021): dates, "who-confirmed-when", real-case retellings → `docs/harness-changelog.md`; keep only rule + why + navigation pointers (file/ADR/spec paths, or terse `(real case: changelog <date>)`). Reason: injected every session, so provenance re-bloats the R14 40KB budget.
...
- **Untracked**: `.agents/projects/` is installation-specific like REGISTRY.md — machines host different projects, so gitignored (extends ADR 005). Sub-harnesses thus live in no git. The user accepted the no-history/no-remote-recovery trade-off.
...
**First-class principle — user comprehension is a completion requirement** (also an always-on CLAUDE.md anchor): the LLM producing correct work is necessary but **not sufficient** — work is not "done"/"delivered" until the user **genuinely understands what was built and why**. "Done" has two halves: the LLM's evidence that the work is correct (§13-2, four fields) **and** the user's demonstrated understanding (the comprehension quiz — item 0 / branch-workflow finish 4). The quiz is the enforcement, **not a formality**: a failed or **skipped** comprehension check blocks delivery (the PR) exactly as a failing test blocks §13-2 — it is a delivery gate (scoped, like all of §13, to feature/behavior work — behavior-invariant trivia is exempt), and the §3/data-loss carve-outs (which permit necessary questions) do not weaken it. This is *why* the harness teaches decisions (surfaces the decision space) and hands a diff reading-guide rather than just shipping output: the goal is the user growing alongside, not the LLM doing everything.

0. **Interview-first (all work) — then zero mid-work questions**: for **every work item, code or not** (this item alone is broader than §13's code scope; trivial behavior-invariant fixes exempt), front-load the full interview before starting. ⓐ Enumerate the prompt's **implicit assumptions, missing constraints, judgment criteria**, plus a **mid-work blocker pre-scan** (access/permissions, environment prerequisites, undecided values — thresholds, names, periods — and ambiguous completion criteria); ⓑ **when uncertain, ask — uncertainty defaults to interviewing** (AskUserQuestion), batching every question the work will need into this single up-front round; ⓒ things findable by lookup (reading code/docs) are looked up, never asked — lookup sharpens questions, it does not replace them. **Never fix the approach first and append questions after.** **Once work starts, ask the user nothing**: resolve in-flight forks yourself by choosing the most suitable, recommendable option, and surface each autonomous decision (choice + rationale) in the completion report and the comprehension quiz (primary run point: the review window, in parallel with the background reviewer; pre-PR fallback + delta re-quiz: branch-workflow finish 4 — the quiz spec single source). **Co-growth (the axis the quiz and teaching reports serve)**: this harness's goal is the user growing alongside, not the LLM doing everything — so decisions are **taught, not just made**. ① Surfaced autonomous decisions and interview forks carry the **decision space** (what forked — A vs B — and why this pick), not only the outcome. ② With the quiz the user is handed a **diff reading-guide** to read before answering (read the change in this order, why each part matters — doc-writer PR template §5's 변경 내용/리뷰 포인트), so review builds real code-reading skill rather than rubber-stamping. Both are cheap report-content additions (§16 — skip the note on self-evident decisions), not new gates. Carve-outs that are mandated confirmations, not scoping questions — never skipped: §3 guardrail confirmations (destructive operations, `[no-test]`/`[secret-ok]`) and **data-loss-preventing confirmations** (uncommitted-changes handling, rebase-conflict resolution direction — branch-workflow; auto-resolving these loses user work no report can recover). If interview premises collapse mid-work, still do not ask — pick the best direction and flag it prominently in the report/quiz. Executor: `architect` (definition §5); direct-execution and generate-verify paths skipping architect (orchestrate Phase 0) also run the interview before starting. Reason: implementing a shallow prompt as-is means 'solving the wrong problem precisely' — one up-front interview round prevents N rework rounds, and banning mid-work questions moves all user latency to one slot, keeping execution unattended-safe.
...
5. **Dark launch for speculative behavior (production/shared hot paths)**: speculative features — behavior-changing heuristics such as threshold triggers, model routing, auto-escalation — touching a hot path (production or shared) ship **feature-flagged, default OFF**; turn ON only after real-environment measurement validates the spec's performance and trigger-frequency assumptions. **An evidence field ④ ("not verified") item that affects production behavior is itself the activation gate — never ship enabled with "verify post-deploy"** (unit tests and review PASS prove logic correctness, not runtime performance/trigger safety). Enabling the flag is its own release step (release skill runbook). Real case: changelog 2026-07-23.
...
- **Harness operational assets the model loads every session/dispatch** (ADR 030): root AGENTS.md, CLAUDE.md, agent definitions (`.agents/agents/*.md`), skills (SKILL.md + references). These are the always-on/high-frequency input-token surface — the largest savings lever.
