...
Why enforcement rather than another written rule: measured 2026-08-03 across this installation's local subagent logs, **this table had never been applied** — dispatches simply inherited the session model, so which tier was in violation flipped with the session. Through 07-22, explore was mostly compliant (11 standard / 4 top) while **25 design-tier runs went out on a standard model** (25 standard / 11 top) — the false-negative state the lightweight ban exists to forbid. From 07-25 the session sat on a top model and explore inverted to 21 top / 3 standard. Counts are state-dependent and rise as sessions accumulate; what reproduces is the inversion and its single cause. The reminder-hook precedent showed where a fix has to sit: agent *type* leaked until it was named at the dispatch point in 2026-07-17, and has held since. Spec: `docs/specs/2026-08-03-tier-gate-hook.md`.
