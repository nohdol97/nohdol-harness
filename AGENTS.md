...
3. **Review against the spec**: reviews (`team-review` skill, reviewer agent) judge against the spec's completion criteria — criteria on paper make judgments reproducible. **Dispatch does not branch on the install-site profile** (ADR 045 superseding 038·042): a `사내` site collects, implements, diagnoses, and verifies with the same subagents a personal one does, and self-review is no more admissible there than here. §5's editing ban is a separate axis and still holds.
