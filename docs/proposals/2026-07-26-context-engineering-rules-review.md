...
- SuperClaude: 2026-07-17 tool-audit이 51,331 메시지 전량 0건 실측 후 제거(세션당 ~36k 토큰). 사용자 전역 CLAUDE.md에 제거 사실 기재됨
...
- **Layer 1 — 적용 완료(2026-07-26).** metaskill 경유, 독립 reviewer PASS. **ADR 개정은 하지 않았다**: 이 변경은 미설치 도구 고유명을 일반 범주로 되돌린 사실 오류 정정이고, 앵커의 발화 조건·적용 범위·계층 규율은 그대로여서 ADR 021이 기록한 결정 자체가 바뀌지 않았다. ADR이 필요한 것은 Layer 2다(앵커를 실제로 줄이면 "무엇을 앵커로 올릴지"의 기준이 바뀐다)
- **Layer 2 — 보류.** 사용자가 Layer 1만 승인(2026-07-26). 적용하려면 ADR 021 개정(앵커 계층 규율 + 복원 조건 명시)이 선행 조건이다
