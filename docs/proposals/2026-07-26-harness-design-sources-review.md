...
> 상태: **전건 규칙 무변경** — 기각 4건·부분 채택 1건, 규칙 변경 후보 1건은 관측 후 규칙화로 보류(2026-07-26 사용자 결정)
> 날짜: 2026-07-26
...
### 결정 — 관측 후 규칙화 (2026-07-26, 사용자)

**적용하지 않는다.** 사유는 §8의 선제 생성 금지·관측 후 승격 원칙이다. 실제로 걸릴 후보 장치는 두 건뿐이고(아래), **모델 업그레이드 후 죽은 장치 때문에 손해를 본 관측은 아직 0건**이다 — ADR 021 건은 손해를 본 것이 아니라 발견한 것이다. 이 규모라면 규칙을 세우는 비용이 그때그때 처리하는 비용보다 크다.

현재 확인된 후보 장치 2건:

| 장치 | 전제된 모델 행동 | 상태 |
|---|---|---|
| ADR 021 상시 앵커의 "주목도 희석 방어" | 긴 문서 속 개별 규칙을 놓치는 position bias | **이미 1건 적중** — A-1에서 1차 출처가 이 일반론을 무효화. Layer 2(순수 중복 앵커 1,039B) 보류 중 |
| ADR 028 gate-reminder 훅 | 진단→구현 미끄러짐, 자가 정정이 트리거 조건에 갇힘 | ADR 본문 근거가 *"프롬프트 문구만으로는 전환 시점을 붙잡지 못했음이 실측됐으므로"* |

**이 결정의 알려진 약점과 그 처리**: 발화 조건이 없으면 정작 모델 업그레이드 시점에 아무도 이 문서를 열지 않는다. 제안 문서는 상황이 아니라 주제로 색인되기 때문이다. 그래서 규칙 대신 **상황 기반 메모리**에 남겼다 — `model-upgrade-dead-workarounds`(§8의 "재발 방지 교훈은 1회차에 기록", MEMORY.md 색인 줄이 발화 상황을 지목). 메모리는 모델 업그레이드 후 하네스를 손대는 상황에서 떠오르므로, 규칙을 만들지 않고도 발견 경로가 유지된다.

**규칙화 재검토 조건**: 모델 업그레이드 후 죽은 보정 장치 때문에 실제 손해(오발화로 인한 마찰, 무효가 된 규칙에 따른 오판정)가 관측될 때. 그때 §8 신호 ②/④ 근거로 `harness-review` 주간 ④ 안의 발화 조건으로 제안한다 — §8이 "신호 집합 고정"을 명시하므로 5번째 신호 신설은 그때도 안 된다.
...
- **보류된 규칙 변경**: 1건 — 4절, 모델 업그레이드 시 우회 장치 재검토. **관측 후 규칙화**로 결정(2026-07-26). 발화 경로는 규칙이 아니라 메모리 `model-upgrade-dead-workarounds`
