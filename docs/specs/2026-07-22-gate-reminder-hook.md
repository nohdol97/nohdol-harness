...
- Codex 세션 커버 — Codex hooks에는 PreToolUse 상당 이벤트가 없어 등록하지 않는다(ADR 019의 SessionStart 파리티와 다름). Codex 세션은 AGENTS.md §7-3 문구(네이티브 로드)가 담당한다. tdd-gate가 git 계층으로 이동한 것(ADR 014·015)과 달리 이 훅은 **커밋 산출물이 아니라 세션 중 라우팅 행동**을 교정하므로 git 훅으로 옮길 수 없다 — 도구 한정 계층이 정당한 경우다.
