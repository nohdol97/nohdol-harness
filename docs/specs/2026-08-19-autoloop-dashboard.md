...
- [x] **C13 (전체 회귀)**: `driver_test.py` 93건, `dashboard_test.py` 19건, 다이어그램 검사 1블록, 하네스 무결성 검사 55건이 모두 통과한다.
