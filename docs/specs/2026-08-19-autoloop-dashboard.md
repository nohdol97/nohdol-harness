...
- 일반 interactive orchestrate 팀을 제어하지 않는다. autoloop 자체의 구조화 task/agent artifact는 읽기 전용으로 투영한다.
...
- **R6 (단계 갱신)**: 드라이버는 적어도 `starting`, `planning`, `dispatching`, `integrating`, `testing`, `verifying`, `finished` 단계를 기록한다. 기록 실패는 `driver.log`에 경고를 남기고 기존 루프의 종료 사유와 게이트 판정에는 영향을 주지 않는다.
...
- **R14 (요구사항 분해)**: autoloop은 편집 전에 스펙 완료 기준을 구조화 task DAG로 나누고, 각 task에 완료 기준 번호, `depends_on`, 기대 검증 증거를 연결한다. `orchestration.json`이 정본이며 driver는 ready task만 선택한다.
- **R15 (task/agent 투영)**: 수집기는 `orchestration.json`에서 verdict·agent budget·task/agent status 집계, task ID, criterion IDs, dependency, owner/agent, requested/effective engine과 fallback, worktree, blocker, timestamps, observed evidence와 bounded dispatch/fallback·integration·cleanup 필드만 반환한다. 상세 화면은 이 값을 `textContent`로 표시한다.
...
flowchart LR
  ...
  DRIVER --> ORCH["orchestration.json<br/>task · dispatch · integration · worktree"]
...
- `GET /api/tasks/<작업명>`
  - 응답: 요약 필드 + `iterations`, `carryover`, `log_tail`, `diagnostics`, `tasks`, `agents`, `dispatches`, `integrations`, `worktrees`
  - `tasks`·`agents`에는 requested/effective engine과 fallback 사유가 포함되고, 나머지 세 배열은 bounded wave·시각·fan-in 결과·retained cleanup 상태만 노출한다.
- `run-status.json`
  - `status`: `running | finished`
  - `phase`: `starting | planning | dispatching | integrating | implementing | testing | verifying | finished`
...
- [x] **C6 (R6)**: 구조화 정상 완료 실행은 `starting → planning → dispatching → testing → verifying → finished`를 기록하고, writer wave는 `integrating`을 추가하며 최종 상태에 `done`이 남는다. 기존 단일 세션 회귀는 `implementing`을 계속 허용한다.
...
- [x] **C13 (전체 회귀)**: `driver_test.py` 132건, `dashboard_test.py` 20건, 다이어그램 검사 2블록, 하네스 무결성 검사 55건이 모두 통과한다.
...
- [x] **C17 (R15)**: 구조화 artifact가 있는 작업의 상세 API와 화면은 task·dependency·agent·dispatch/fallback·integration·worktree cleanup·evidence를 표시하고, artifact가 없는 과거 작업은 기존 aggregate 화면을 유지한다.
...
| 2026-08-19 | task DAG·agent·worktree·evidence 투영 추가 | 대시보드 수집기·화면·테스트 | ADR 048의 무인 오케스트레이션 상태를 aggregate가 아닌 task 단위로 관찰하기 위함 |
| 2026-08-19 | dispatch/fallback·integration·cleanup 투영과 구조화 phase 계약 보강 | 대시보드 API·화면·테스트·이 문서 | 독립 검토에서 aggregate task 표만으로는 병렬 배치와 fan-in 실패를 추적할 수 없다는 DOC-01·DOC-04를 확인 |
