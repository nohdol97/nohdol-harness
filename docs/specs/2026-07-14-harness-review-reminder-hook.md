...
| 2026-08-03 | 변이 테스트 지적 반영 — 절 종료를 모든 레벨 제목(`#`~`######`)으로 확장(B3), 절 한정을 실제로 고정하는 순서로 테스트 재배치(B2). 리마인더 지시문에 explore 티어 추론 등급 명시 | R8, `.agents/hooks/harness-review-reminder.py`, `.agents/hooks/harness-review-reminder_test.py` | 독립 검증 B2: 프로필 절을 파일 앞에 두고 검사해 **절 한정을 제거한 변이가 36/36 통과로 살아남았다** — 테스트가 그 축을 전혀 고정하지 못했다. B3: `## `만 절을 닫아 `#`·`###` 뒤의 굵은 라벨을 프로필로 읽었고, 이는 그 설치처가 아닌 값으로 일일 점검을 끄는 억제 방향 위양성이다(재현 후 수정, 회귀 37건 OK) |
