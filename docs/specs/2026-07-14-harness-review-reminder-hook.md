...
- R3: 모드 판정 — 전체 마커 7일 이상(또는 기록 없음)이면 **full**, 아니고 일일 마커 1일 이상(또는 기록 없음)이면 **daily**, 둘 다 아니면 **상태 한 줄**(점검 지시 아님 — 마지막 점검 시점, 기한 전 안내, `_workspace/harness-ops-log.md` 참조, 사용자 보고 지시). full·daily 지시에는 **첫 응답에서 사용자 요청 처리에 앞선 우선 실행**(미루려면 사용자에게 명시적 확인)과 마커 갱신 안내, 점검 결과의 채팅 보고·운영 로그 기록 안내를 포함한다 — 훅은 모델을 깨우지 못하므로 지시는 첫 사용자 턴에서 소비되는데, "지금 실행" 수준의 문구는 사용자 요청과 경합할 때 밀린다(2026-07-14 관찰).
