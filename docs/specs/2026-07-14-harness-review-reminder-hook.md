...
- 둘 다 기한 내면 완전히 침묵한다. 사용자가 기억할 필요가 없다.
...
- R3: 모드 판정 — 전체 마커 7일 이상(또는 기록 없음)이면 **full**, 아니고 일일 마커 1일 이상(또는 기록 없음)이면 **daily**, 둘 다 아니면 무출력. full 지시에는 완료 시 두 마커 모두 갱신, daily 지시에는 일일 마커 갱신 안내를 포함한다.
