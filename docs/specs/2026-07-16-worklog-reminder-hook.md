...
- 다음 세션 **SessionStart에 로컬 git으로 이전 세션이 남긴 미커밋 작업을 감지**해, 있으면 work-tracker 흐름 2(진행 로그) / 흐름 3(재개)을 환기하는 한 줄을 세션 컨텍스트에 주입한다. 미커밋 변경이 없으면 **침묵**한다 — 이 훅의 "정상"은 깨끗한 트리이고, 매 세션 출력은 노이즈다(harness-review-reminder도 2026-07-16부터 기한 전엔 무출력으로 정렬됐다 — SessionStart 훅의 nominal 케이스는 침묵이 기본).
