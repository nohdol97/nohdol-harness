...
- R4: fail-open — status 타임아웃(10초)·예외·기동 실패 전부 exit 0, **예외 경로는 무출력**. 정상 경로(미설치·실행 중·기동함)는 **상태 한 줄**을 출력한다 — 훅이 동작했다는 증거가 채팅에 남아야 하기 때문(2026-07-14 사용자 요청. 구판의 전면 무출력은 정상과 미실행을 구분 불가).
