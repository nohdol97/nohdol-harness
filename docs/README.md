...
| [038](adr/038-corporate-profile-verification-exemption.md) | 2026-08-03 | **폐기(→045)** | 사내 프로필의 검증 목적 발행 면제 — reviewer·관점 팬아웃·리뷰 fan-in integrator·infra-specialist 리뷰 모드를 끄고 메인 루프 자체 검증+면제 기록으로 대체, 명시 요청은 실행. 훅(`review-gate`)이 막는 것은 `reviewer` 타입 하나 |
...
| [045](adr/045-corporate-profile-dispatch-restored.md) | 2026-08-07 | 활성 | **사내 프로필의 서브에이전트 발행 차단 폐기**(042·038 대체) — 설치처 프로필은 더 이상 발행의 판정 입력이 아니며, 그 사이트도 개인과 같은 역할·팬아웃·독립 검증을 쓴다. `dispatch-gate.py`와 테스트·등록·무결성 필수 목록을 삭제하고 `infra-specialist` 예외도 함께 소멸(전부 통과하므로 뜻이 없다). **유지**: §5 추적 파일 편집 금지(ADR 012), §8 일일 점검 off, ADR 043 worktree 폐기 — 근거가 각각 편집 권한·적용 가능성·git 동작이라 발행 비용 축이 아니다. PR 본문 `독립 검증` 줄과 운영 로그 3상태는 유지하되 프로필 분기를 뺀 문구로 일반화 |
...
| [2026-08-04-dispatch-gate-hook](specs/2026-08-04-dispatch-gate-hook.md) | **폐기(→045)** | (훅 삭제됨) | 038·042·045 |
