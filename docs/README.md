...
| [2026-07-26-harness-design-sources-review](proposals/2026-07-26-harness-design-sources-review.md) | 대부분 기각(규칙 변경 후보 1건 승인 대기) | 하네스 설계 자료 5건(이슈 #41 A-2~A-6) 1차 출처 대조. A-2 OpenAI 하네스 엔지니어링 기각(§11 Codex 축 미접촉·자동 머지 등 §5·§13 정면 충돌·원문 자체가 고처리량 한정), A-3 Anthropic 장기 실행 부분 채택(generate-verify 불변식 보강, 단 품질 주장은 전부 N=1 미반복), A-4 HyperAgents 기각(**논문에 "harness" 0회 — 표제가 블로거 프레임**·CC BY-NC-SA·100반복 8,860만 토큰), A-5 Managed Agents 기각(두뇌/손은 모델 분리가 아니라 하네스/샌드박스 분리라 §9 근거 불가), A-6 동적 워크플로우 기각(실축이 §7이고 orchestrate 기수렴). **유일한 규칙 변경 후보: A-3·A-5가 독립 도달한 "모델 업그레이드 시 우회 장치는 죽은 코드가 된다" → 신호 ④ 주간 점검에 모델 버전 변경 발화 조건 편입(신호 신설 아님)** |
