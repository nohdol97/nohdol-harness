...
| [2026-07-25-claude-video-adoption](proposals/2026-07-25-claude-video-adoption.md) | 채택(직접 호출 전용 — harness-install 3h) | claude-video `/watch` 영상 이해 스킬 — 순수 신규 역량(중복 0)·opt-in-per-use. Whisper 폴백이 오디오를 제3자 전송하는 게 유일 §3 유출, 사내·민감 녹화는 `--no-whisper`/자막 전용 |
