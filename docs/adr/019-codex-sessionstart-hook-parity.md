...
> **부분 대체(→029·031, 2026-07-25)**: 확인된 이벤트만 등록한다는 자세와 agentsview-daemon 제외는 ADR 029가, `.codex/hooks.json` 위치·`codex_hooks` 키·clone 즉시 활성 주장은 ADR 031이 대체했다. SessionStart 파리티·공용 스크립트·`$PWD` 경로·macOS/Linux 범위는 유효하다.
