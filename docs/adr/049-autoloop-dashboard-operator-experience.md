...
> **후속 정교화(2026-08-20)**: 초기 접힘 원칙은 유지하되, 펼친 뒤에도 내부 필드가 한 줄로 나열되지 않도록 결정 2·6을 보완했다. 세부 정보는 의미별 카드·한국어 라벨·상태 요약으로 보여 주고 경로·commit·원문 log만 고정폭으로 보존한다.

> **후속 정교화(2026-08-20, 정보 중복 제거)**: 결정 2·3·6·10을 보완했다. 핵심 lifecycle은 T 핸드오프 한 곳에 합치고, 같은 task lifecycle의 중복 감사 event는 strip에서만 한 번으로 압축한다. 실행 조율은 실행 묶음·선행 대기·조정만 맡고, agent 기술 정보는 단일 접힌 세부 정보만 맡는다.

> **부분 대체(2026-08-20, 실행 시간축 제거)**: 사용자 판단에 따라 결정 2·6·8·9에서 `한 줄 실행 시간축`을 제거했다. 기본 상세는 최소 개요·T 핸드오프·실행 조율만 남고, agent 실행 시각은 `실행 세부 정보 > 에이전트`에서만 제공한다. 축이 유일한 inline style 소비처였으므로 CSP `style-src`의 `'unsafe-inline'`도 함께 회수한다.
...
2. 상세 기본 구조는 `최소 개요`, `T 핸드오프`, `실행 조율 요약`으로 제한한다. 실행 조율은 실행 묶음·선행 작업 대기·실행 조정만 답하며 handoff event 수·최신 event·전체 event 목록을 반복하지 않는다. 비용·engine·worktree·integration·commit·evidence·diagnostics는 초기 접힘 세부 정보로 이동한다. 펼친 내용은 에이전트·통합 결과·작업 공간·실행 기록·확인할 문제로 나누고, 한국어 라벨과 상태 요약을 사용한다.
3. task 관계는 그래프 대신 기록된 `team-log.jsonl`의 `team_create`·task lifecycle·`integration_complete` event를 시간순 chip과 방향 cue로 잇는다. 같은 task·wave·event가 통합 전후에 반복되면 원본 감사 기록은 보존하되 strip에서는 한 번만 표시하고, 같은 timestamp에서는 팀 생성→작업 시작→작업 완료/실패→통합 완료 순으로 정렬한다. 이는 대화가 아니라 기록된 작업 전이이며, event가 없을 때 dependency로 추정하지 않는다.
...
6. agent 실행은 기본 화면에 축으로 그리지 않는다. 역할·tier·model·requested/effective engine·실행 시간·fallback은 하나의 `실행 세부 정보 > 에이전트`에서만 제공하며, 기본 화면에는 agent 전용 영역을 두지 않는다. worktree·integration 정보도 같은 접힘 안에서 기록값을 보존하되 사람이 읽는 제목·라벨·상태 문장으로 제공한다.
7. 자동 갱신은 선택·상세 위치·포커스를 보존하고 신선도와 갱신 실패를 실행 실패와 분리한다. 색만으로 상태를 전달하지 않으며 키보드·스크린리더·reduced motion·200% 확대를 완료 조건에 포함한다.
8. ADR 047의 loopback·읽기 전용·Host/method/path/symlink·보안 헤더 계약과 ADR 048의 artifact 역할 분리를 유지한다. API 변경은 additive이고, 외부 문자열은 escaped React text child로만 렌더링한다. unsafe HTML·parsing·실행·style·URL sink를 금지하고 read는 bounded다. CSP `style-src`는 inline style을 허용하지 않는다.
9. React·TypeScript·Vite source와 lockfile을 두고 production static build를 커밋한다. runtime frontend 의존성은 React·React DOM만 남기며 handoff strip은 CSS로 만든다. Node/npm은 개발·빌드·테스트 의존성이고 dashboard runtime은 계속 Python 표준 라이브러리만 요구한다. CDN·remote asset·server-side 사용자 상태 저장은 도입하지 않는다.
10. 구현과 독립 검증의 정본은 `docs/specs/2026-08-19-autoloop-dashboard-operator-ux.md`의 R1~R19와 C1~C22이다. 이 ADR은 설계 선택을, 해당 스펙은 테스트 가능한 동작을 소유한다.
...
- 사용자는 첫 화면에서 주의 작업을 찾고 T 핸드오프 → 실행 조율 요약을 같은 맥락에서 추적할 수 있다.
...
- Light를 첫 방문 기본값으로 두고 System·Dark를 선택할 수 있으며, handoff strip이 운영 상태를 시각적으로 설명한다.
