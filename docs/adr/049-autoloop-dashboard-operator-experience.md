...
3. task dependency는 React Flow와 Dagre로 실제 node·방향성 edge·결정적 layout을 제공하고, 동일 정보를 담은 키보드 접근 가능 표 또는 순서 목록을 함께 제공한다.
...
9. React·TypeScript·Vite source와 lockfile을 두고 production static build를 커밋한다. React Flow·Dagre는 task graph에만 사용한다. Node/npm은 개발·빌드·테스트 의존성이고 dashboard runtime은 계속 Python 표준 라이브러리만 요구한다. CDN·remote asset·server-side 사용자 상태 저장은 도입하지 않는다.
...
- Light를 첫 방문 기본값으로 두고 System·Dark를 선택할 수 있으며, graph·agent lane·event·cost가 운영 상태를 시각적으로 설명한다.
