...
2. 상세 기본 구조는 `최소 개요`, `T 핸드오프`, `한 줄 실행 시간축`, `실행 조율 요약`으로 제한한다. 실행 조율은 실행 묶음·선행 작업 대기·실행 조정만 답하며 handoff event 수·최신 event·전체 event 목록을 반복하지 않는다. 비용·engine·worktree·integration·commit·evidence·diagnostics는 초기 접힘 세부 정보로 이동한다. 펼친 내용은 에이전트·통합 결과·작업 공간·실행 기록·확인할 문제로 나누고, 한국어 라벨과 상태 요약을 사용한다.
...
6. agent 기본 화면은 모든 실행을 하나의 공통 시간축에 놓고 ID·상태만 보여 준다. 실제 시간이 겹치는 실행은 X 위치를 바꾸지 않고 같은 축 안의 layer로 쌓아 병렬 관계를 보존한다. 시간축 아래에 별도 agent disclosure를 반복하지 않고, 역할·tier·model·requested/effective engine·실행 시간·fallback은 하나의 `실행 세부 정보 > 에이전트`에서만 제공한다. worktree·integration 정보도 같은 접힘 안에서 기록값을 보존하되 사람이 읽는 제목·라벨·상태 문장으로 제공한다.
...
- 사용자는 첫 화면에서 주의 작업을 찾고 T 핸드오프 → 한 줄 실행 시간축 → 실행 조율 요약을 같은 맥락에서 추적할 수 있다.
