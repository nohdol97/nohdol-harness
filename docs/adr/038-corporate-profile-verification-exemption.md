...
- **날짜**: 2026-08-03
- **상태**: **폐기(→045)** — 먼저 [ADR 042](042-corporate-profile-dispatch-block.md)가 차단 **범위**와 우회 표식을 대체했고(검증 축 → 발행 축 전체, `[review-ok]` 폐기, 훅 개명 `review-gate.py`→`dispatch-gate.py`), 이어 [ADR 045](045-corporate-profile-dispatch-restored.md)가 **둘 다 걷었다**(2026-08-07). 042만 되돌리면 이 ADR의 검증 축 차단이 되살아나므로 함께 폐기한 것이다. 사내 프로필에 발행 면제는 더 이상 없다. **살아남은 것은 기록 의무 쪽뿐이다** — PR 본문 `독립 검증` 줄과 하네스 변경 이력 행은 유지되되, 붙잡는 대상이 "면제였는가"가 아니라 "검증이 돌았는가"로 일반화됐다. 아래 내용 전체가 시점 기록이며 현재 규칙이 아니다.
